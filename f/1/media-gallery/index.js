$(document).ready(function(){      
    ...
    $(window).scroll(function() {
        if($(window).scrollTop() + $(window).height() >= $(document).height()  && !inProgress && page <= count) {
            var url = "/ajax/getMediaCenter.php?PAGEN_1="+page;
            if($('.filter_section .selected').attr('section') != 'all'){
                url = "/ajax/getMediaCenter.php?PAGEN_1="+page+"&SECTION_CODE="+$('.filter_section .selected').attr('section');
            }
            inProgress = true;
            $.ajax({
                url: url,
    ...
    $('.switcher_item').on('click', function(){
        if(!$(this).hasClass('selected')){
            if($(this).attr('section') != 'all'){
                url = "/ajax/getMediaCenter.php?PAGEN_1=1&SECTION_CODE="+$(this).attr('section');
                inProgress = true;
                $.ajax({
                    url: url,
                    type: "GET",
                ...
                url = "/ajax/getMediaCenter.php?PAGEN_1=1";
                inProgress = true;
                $.ajax({
                    url: url,
                    type: "GET",
