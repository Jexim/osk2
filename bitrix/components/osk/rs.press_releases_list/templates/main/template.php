...
$APPLICATION->AddHeadScript('/f/1/press-releases/index.js');
$APPLICATION->SetAdditionalCSS('/f/1/press-releases/index.css');
?>
        ...
        <div class="list_search_block">
            <div class="form_validate field required" data-id="field">
                <form method="GET" action="/search/">
                    <input type="hidden" value="press_releases" name="TYPE" />
                    <input type="text" placeholder="Поиск в пресс-релизах" value="<?=$_REQUEST['SEARCH_TEXT']?>" name="SEARCH_TEXT"><button class="small disabled b_submit">Найти</button>
                </form>
            </div>
            <? if(empty($_REQUEST['SEARCH_TEXT'])): ?>
                <div class="filter_year">
                    <? foreach ($arResult['YEAR'] as $year):?>
                        ...
                        <? else: ?>
                            <div class="item"><a href="/press-center/press-releases/<?=$year?>"><?=$year?></a></div>
                        <? endif; ?>
                    <? endforeach; ?>
                </div>
