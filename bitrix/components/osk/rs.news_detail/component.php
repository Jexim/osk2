...
if (CModule::IncludeModule("iblock")) {    
    ...
    //Получаем новость
    $arSelect = Array("*");
    $arFilter = Array("IBLOCK_CODE"=>"news", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "CODE"=>$_REQUEST['ELEMENT_CODE']);
    ...
    //Получаем звязанние новости
    $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PREVIEW_TEXT", "DETAIL_PICTURE", "DETAIL_PAGE_URL"); 
    $arFilter = Array("IBLOCK_CODE"=>"news", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID"=>$arResult['ELEMENT']['PROP']['RELATED_NEWS']['VALUE']);
    $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
    while($ob = $res->GetNext()){
        $arResult['NEWS'][$ob['ID']] = $ob;
        $arResult['NEWS'][$ob['ID']]['DATE'] = FormatDateFromDB($ob['DATE_ACTIVE_FROM'], 'DD MMMM');
    }
    //Получаем звязанние пресс-релизы
    $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PREVIEW_TEXT", "DETAIL_PICTURE", "DETAIL_PAGE_URL"); 
    $arFilter = Array("IBLOCK_CODE"=>"press_releases", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID"=>$arResult['ELEMENT']['PROP']['RELATED_PRESS_RELEASES']['VALUE']);
    $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
    while($ob = $res->GetNext()){
        $arResult['PRESS_RELEASES'][$ob['ID']] = $ob;
        $arResult['PRESS_RELEASES'][$ob['ID']]['DATE'] = FormatDateFromDB($ob['DATE_ACTIVE_FROM'], 'DD MMMM');
    }
