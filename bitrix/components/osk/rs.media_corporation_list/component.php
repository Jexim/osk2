...
if (CModule::IncludeModule("iblock")) {            
    ...
    if(empty($_REQUEST['MONTH'])){
        //Новости всего года
        $arFilter = Array("IBLOCK_CODE"=>"media_corporation", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", ">=DATE_ACTIVE_FROM" => "01.01." . $arResult['THIS_YEAR'], "<=DATE_ACTIVE_FROM" => "31.12." . $arResult['THIS_YEAR']);
        
    } else {
        //Новости заданого месяца    
        $arFilter = Array("IBLOCK_CODE"=>"media_corporation", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", ">=DATE_ACTIVE_FROM" => "01." . $_REQUEST['MONTH'] . "." . $arResult['THIS_YEAR'], "<=DATE_ACTIVE_FROM" => "31." . $_REQUEST['MONTH'] . "." . $arResult['THIS_YEAR']);
    }
    //Получаем массив новостей
    $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PREVIEW_TEXT", "DETAIL_PICTURE", "DETAIL_PAGE_URL", "PROPERTY_NAME_PUBLICATION", "PROPERTY_LINK_PUBLICATION");   
    ...
    $arResult['PAGINATION'] = $res->GetPageNavStringEx($navComponentObject, "", "media_corporation");
    //Получаем года новостей
    $arSelect = Array("DATE_ACTIVE_FROM");
    $arFilter = Array("IBLOCK_CODE"=>"media_corporation", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
