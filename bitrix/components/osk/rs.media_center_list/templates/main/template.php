...
$APPLICATION->AddHeadScript('/f/1/media-gallery/index.js');
$APPLICATION->SetAdditionalCSS('/f/1/media-gallery/index.css');
?>

    <div class="block clear">
        <div class="sub_filter filter_section">
            <? if($arResult['ALL_CATEGORY'] || count($arResult['SECTION'])!=1): ?>
                <div class="item switcher_item for_jn_view selected" section="all"><span class="pseudo">Все материалы</span></div>
            <? endif; ?>
            <? foreach ($arResult['SECTION'] as $section): ?>
                <div class="item switcher_item for_jn_view <? if(!$arResult['ALL_CATEGORY'] && count($arResult['SECTION'])==1): ?>selected<? endif; ?>" section="<?= $section['CODE'] ?>"><span class="pseudo"><?= $section['NAME'] ?></span></div>
            <? endforeach; ?>
        </div>
    </div>
            ...
            <? foreach ($arResult['ELEMENTS'] as $value): ?>
                <? if($value['ITEM']['IBLOCK_CODE']=='photo'): ?>
                    <? if(!empty($value['ITEM']['PREVIEW_PICTURE'])): ?>
                        <? $pach = CFile::ResizeImageGet($value['ITEM']['PREVIEW_PICTURE'], array('width'=>650, 'height'=>500), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true); ?>
                    <? else: ?>
